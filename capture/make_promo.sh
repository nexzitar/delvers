#!/bin/zsh
# Assembles the promo into docs/trailer/delvers_promo.mp4.
# Preferred: Movie Maker master (true 30fps + audio):
#   godot --path . --write-movie capture/proto3d/renders/promo/master.avi \\
#     --fixed-fps 30 --resolution 1920x1080 res://capture/promo_shot.tscn
# Fallback: PNG segment dirs from a realtime run of promo_shot.tscn.
set -e
cd "$(dirname "$0")/.."
mkdir -p docs/trailer capture/proto3d/renders/promo/encoded

python3 - <<'EOF'
import json, os, subprocess

src = "capture/proto3d/renders/promo"
...
subprocess.run([
    ...
    "docs/trailer/delvers_promo.mp4",
], check=True, capture_output=True)
print("promo assembled: docs/trailer/delvers_promo.mp4")
EOF
